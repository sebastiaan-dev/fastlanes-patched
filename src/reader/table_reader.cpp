...
up<RowgroupReader> TableReader::get_rowgroup_reader(const n_t                 rowgroup_idx,
	...
	auto rowgroup_reader = make_unique<RowgroupReader>(io, *rg, m_connection, column_ids, m_buffer_pool);
...
up<RowgroupReader> TableReader::get_rowgroup_reader(const n_t rowgroup_idx) const {
	...
	const auto             fb_idx = static_cast<flatbuffers::uoffset_t>(rowgroup_idx);
	const auto*            rg     = td->m_rowgroup_descriptors()->Get(fb_idx); // pointer to RowgroupDescriptor (table)
	return make_unique<RowgroupReader>(m_file_path, *rg, m_connection, m_buffer_pool);
}
...
TableReader::TableReader(const path& file_path, Connection& connection)
    ...
    , m_buffer_pool(std::make_shared<BufPool>()) {
	...
	if (file_header.settings.inline_footer) {
		...
		m_table_descriptor_handle =
		    make_table_descriptor(io, file_footer.table_descriptor_offset, file_footer.table_descriptor_size);
	} else {
